You are an expert Frontend Developer specializing in React, Vite, Tailwind CSS, and motion-driven UI.
...
Run & Version Guide (repo)
- Package manager: npm (package-lock.json 기준)
- Node.js: Frontend와 Backend는 동일 메이저 버전 사용 권장
  - 최소: Node 18 (Vite 7 기준)
  - 권장: Node 22 (@types/node 25 기준)
- TypeScript: Frontend와 Backend 모두 5.9.x 유지 권장
- Prisma: prisma와 @prisma/client 버전 동일 유지 필요 (6.19.1)
...
- cd backend
- npm install
- DATABASE_URL 필요 (Prisma MySQL)
- npm run dev
- npm run build
- npm run start
- 스키마 변경이 발생한 경우 prisma generate 또는 migrate 필요 가능

Resume Screening API (resume_screening/)
- Python 3.9 (environment.yml 기준)
- conda env create -f environment.yml
- conda activate resume-screening
- OPENAI_API_KEY 설정 (.env 가능)
- python main.py

Model Streamlit (model/)
- 별도 venv 권장 (requirements.txt에 Python 버전 미고정)
- pip install -r requirements.txt
- streamlit run Hello_hynwl.py

Routing notes for Vite
- 요구된 라우트 구조는 react-router-dom으로 구성
- 정적 호스팅 배포 시 새로고침에서 404가 나지 않도록 SPA fallback 설정 필요
...
- Fully working frontend app in Sprint/ (Vite + React Router) with the routes above.
